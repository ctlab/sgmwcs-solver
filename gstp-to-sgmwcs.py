...
class ToSgmwcs(object):
  ...
  def __init__(self, cords, edges, terms, out_path):
    edges_out = open(out_path + '/edges', 'w')
    nodes_out = open(out_path + '/nodes', 'w')
    sigs_out = open(out_path + '/signals', 'w')
    ss = {}
    for t in terms:
      ss["S" + str(t)] = 0
    ss['S0'] = 0
...
def main(stp_path, stp_file):
  reader = StpReader(stp_path + "/" + stp_file)
  reader.read()
  ToSgmwcs(reader.cords, reader.edges, reader.terms, stp_path)
